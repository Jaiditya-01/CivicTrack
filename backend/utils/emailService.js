...
async function sendPasswordResetEmail(email, payload) {
  ...
  const { name, resetUrl, expiresMinutes } = payload;
  ...
  const html = `
    ...
    <p>We received a request to reset your CivicTrack password.</p>
  ...
  const info = await trans.sendMail({
    ...
    subject: '[CivicTrack] Reset your password',

    text: `Reset your CivicTrack password using this link: ${resetUrl} (expires in ${expiresMinutes} minutes).`,
  ...
  if (isDevInbox() && nodemailer.getTestMessageUrl) {
    ...
    if (previewUrl) console.log('[Mail] Preview (password reset):', previewUrl);
